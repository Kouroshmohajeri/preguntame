...
const handler = NextAuth({
  ...
  callbacks: {
    async jwt({ token, account, user }) {
      ...
      if (account && user?.email) {
        try {
          ...
          // Create or get user from backend AND GET JWT TOKEN
          const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/users/google`, {
            name: user.name?.split(" ")[0] || "",
            lastname: user.name?.split(" ")[1] || "",
            email: user.email,
          ...
          // Store user ID, email, AND backend JWT token
          token.id = res.data.user._id;
          token.email = user.email;
          token.backendToken = res.data.token; // ✅ Store backend JWT
        } catch (err) {
          console.error("❌ Failed to sync user to database:", err);
        }
      } else if (token.email && !token.id) {
        ...
        try {
          ...
          token.id = res.data.user._id;
          token.backendToken = res.data.token; // ✅ Get fresh JWT
          console.log("✅ User data retrieved:", token.id);
        } catch (err) {
    ...
    async session({ session, token }) {
      // Pass user ID and backend JWT to session
      if (token?.id) {
        session.user.id = token.id as string;
      }
      if (token?.backendToken) {
        session.user.backendToken = token.backendToken as string; // ✅ Add to session
      }
      return session;
    },
  },
